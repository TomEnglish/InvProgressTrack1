...
export async function parseAuditFile(file: File): Promise<ParseResult> {
  ...
  const raw: Record<string, unknown>[] = XLSX.utils.sheet_to_json(workbook.Sheets[sheetName], { defval: '' });
...
export function detectAuditDiscipline(filename: string, disciplines: { id: string; name: string }[]): string | undefined {
